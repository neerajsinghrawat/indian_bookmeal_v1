...
//category routes
	...
	Route::get('/dashboard', 'Front\UsersController@index')->middleware('auth');

    Route::post('/create_newsletter', ['as' => 'pages.create_newsletter.post', 'uses' => 'Front\PagesController@create_newsletter']);
   
    
    //kz 18 dec 2018
    Route::post('/save_complaint', 'Front\UsersController@save_complaint')->middleware('auth');
	Route::post('/users/saveResolvedOrderComplaints', 'Front\UsersController@saveResolvedOrderComplaints')->middleware('auth');
	Route::get('/users/setDashboardTabSession', 'Front\UsersController@setDashboardTabSession')->middleware('auth');
	Route::get('/order/{order_number}', 'Front\OrdersController@order_detail')->middleware('auth');

    Route::get('/about-us', 'Front\PagesController@about_us');
    Route::put('/users/edit_profile/{id}', 'Front\UsersController@edit_profile')->middleware('auth');

    Route::post('/users/change_password',  ['as' => 'users.change_password.post',   'uses' => 'Front\UsersController@change_password'])->middleware('auth');
    ...
    //kz 19 dec 2018
	Route::post('/save_product_review', 'Front\ProductsController@save_product_review')->middleware('auth');
	
	Route::get('/product-tag/{slug}','Front\ProductsController@product_tag_list');

    Route::put('/users/user_address/{id}', 'Front\UsersController@user_address')->middleware('auth');

    Route::post('/users/delete_ajax_address/', 'Front\UsersController@delete_ajax_address')->middleware('auth');

    Route::post('/products/ajaxSelectDeliveryAddress/', 'Front\ProductsController@ajaxSelectDeliveryAddress')->middleware('auth');
    Route::get('/client', 'Front\PagesController@client_list');
    Route::get('/tableReservation', 'Front\PagesController@table_reservation');
    ...
    Route::get('/apply-coupon/{id}/{slug}', 'Front\ProductsController@apply_coupon_amount')->middleware('auth');
    Route::get('/back-to-cart', 'Front\ProductsController@back_to_cart')->middleware('auth');
...
*/
    ...
    Route::get('/productFeatures', 'Admin\ProductFeaturesController@index');
    Route::get('/productFeatures/add', 'Admin\ProductFeaturesController@add');
   ...
   Route::get('/book_table', 'Admin\PagesController@book_table');
    ...
    Route::post('/products/delete_ajax_item/', 'Admin\ProductsController@delete_ajax_item');
    ...
    Route::get('/paymentGetways/edit/', 'Admin\PaymentGetwaysController@edit');
    Route::put('/paymentGetways/edit/', 'Admin\PaymentGetwaysController@update');
    ...
    Route::post('/orders/update_delivery_satff', 'Admin\OrdersController@update_delivery_satff');
        ...
        Route::get('/shippingTaxes', 'Admin\ShippingTaxesController@edit');
    Route::put('/shippingTaxes/edit/{id}', 'Admin\ShippingTaxesController@update');
